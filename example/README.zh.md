...
SDK 下载包中包含了 certificate.bin 文件，因此你无需购买商用 License 即可免费体验 RTSA Pro SDK。免费体验存在并发限制且时效为 90 天。
...
5. 联系声网客户经理(yangdifei@agora.io)，购买商用的License。
...
该程序激活成功后，默认在当前目录生成 **certificate.bin** ，启动应用时会用到它们。其参数如下：
- 参数 `YOUR_APPID` 需要替换为你创建的 App ID.
- 参数 `YOUR_KEY` 需要替换为你刚才保存的 `key_and_secret.txt` 文件里的 key值
- 参数 `YOUR_SECRET` 需要替换为你刚才保存的 `key_and_secret.txt` 文件里的 secret值
